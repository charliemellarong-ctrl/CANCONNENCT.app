...
def show_lgu_services():
    ...
    # Civil Registry
    st.markdown('<div class="service-header">📄 Civil Registry</div>', unsafe_allow_html=True)
    
    civil_services = [
        {"name": "Birth Certificate", "fee": "₱150", "processing": "3-5 days"},
        {"name": "Marriage Certificate", "fee": "₱150", "processing": "3-5 days"},
        {"name": "Death Certificate", "fee": "₱150", "processing": "3-5 days"},
        {"name": "CENOMAR", "fee": "₱210", "processing": "5-7 days"}
    ]
    
    for service in civil_services:
        with st.container():
            st.markdown(f"""
            ...
            """, unsafe_allow_html=True)
            if st.button(f"Apply Now", key=f"civil_{service['name']}"):
                st.session_state['selected_service'] = service
                st.session_state['service_type'] = 'civil'
                show_application_form(service['name'])
    
    # Business and Permits
    st.markdown('<div class="service-header">💼 Business and Permits</div>', unsafe_allow_html=True)
    
    business_services = [
        {"name": "Business Permit", "fee": "₱500-2000", "processing": "7-10 days"},
        ...
        {"name": "Community Tax Certificate", "fee": "₱5-25", "processing": "1 day"}
    ]
    ...
    for service in business_services:
        with st.container():
            ...
            """, unsafe_allow_html=True)
            if st.button(f"Apply Now", key=f"business_{service['name']}"):
                st.session_state['selected_service'] = service
                st.session_state['service_type'] = 'business'
                show_application_form(service['name'])
    
    # Health Agriculture Sanitation
    st.markdown('<div class="service-header">🏥 Health, Agriculture & Sanitation</div>', unsafe_allow_html=True)
    
    health_services = [
        {"name": "Health & Sanitation Clearance", "fee": "₱300", "processing": "3-5 days"},
        {"name": "Veterinary Certificate", "fee": "₱200", "processing": "2-3 days"}
    ]
    ...
    for service in health_services:
        with st.container():
            ...
            """, unsafe_allow_html=True)
            if st.button(f"Apply Now", key=f"health_{service['name']}"):
                st.session_state['selected_service'] = service
                st.session_state['service_type'] = 'health'
                show_application_form(service['name'])
...
def show_application_form(service_name):
    st.markdown("---")
        ...
        if submitted:
            if agree and first_name and last_name and contact_no and email:
                ...
                st.session_state.payments.append({
                    "id": payment_id,
                    "tracking": tracking_no,
                    "amount": 150 if "Certificate" in service_name else 500,
